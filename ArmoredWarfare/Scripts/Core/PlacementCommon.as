...
bool isBuildableAtPos(CBlob@ this, Vec2f p, TileType buildTile, CBlob @blob, bool &out sameTile)
{
	...
	if (backtile.type >= CMap::tile_metal_back && backtile.type <= CMap::tile_metal_back_d8)
	{
		return false;
	}
	...
	else if ((buildTile == CMap::tile_castle || buildTile == CMap::tile_scrap) && backtile.type >= CMap::tile_wood && backtile.type <= CMap::tile_wood_d0 && !map.isInFire(p))
	{
	...
	if (blob is null || !blob.hasTag("ignore blocking actors"))
	{
		...
		{
			// from canGrow.as
			TileType tile = map.getTile(p + Vec2f(0, 8)).type;
			return (map.isTileGround(tile) || isTileCompactedDirt(tile));
