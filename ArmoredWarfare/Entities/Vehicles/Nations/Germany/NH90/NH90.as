...
const Vec2f upVelo = Vec2f(0.00f, -0.0375f);
...
void onInit(CBlob@ this)
{
	...
	this.inventoryButtonPos = Vec2f(-12.0f, -2);
...
{	
	...
	if (tailrotor !is null)
	{
		...
		tailrotor.SetOffset(Vec2f(58, -8));
	...
	CSpriteLayer@ tracks = this.addSpriteLayer("tracks", "NH90_tracks.png", 48, 16);
	if (tracks !is null)
	{
		tracks.SetOffset(Vec2f(-12.0f, 11.0f));
				...
				if (hooman !is null)
				{
					...
					else if (ap.name == "GUNNER")
					{
						...
						if (minigun !is null)
						{
							...
							Vec2f aimvector = GunAimPos - this.getPosition()+Vec2f(this.isFacingLeft() ? 42 : -42, -8).RotateBy(this.getAngleDegrees());
								...
								if (getGameTime() > this.get_u32("fireDelayGun") && hooman.isMyPlayer())
								{
									...
									if (ammocarry !is null && ammocarry.hasBlob("ammo", 1))
									{
										...
										shootVehicleGun(hooman.getNetworkID(), this.getNetworkID(),
											true_angle, this.getPosition()+Vec2f(this.isFacingLeft()?miniGun_offset.x:-miniGun_offset.x, miniGun_offset.y).RotateBy(this.getAngleDegrees()),
