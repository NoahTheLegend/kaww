...
void onInit(CBlob@ this)
{
	...
	{
		this.set_f32(capture_time_prop, capture_half_seconds);
	}
...
{
	this.set_f32(friendly_prop, 0);
	this.set_f32(enemy_prop, 0);
	this.set_f32(counter_prop, this.get_f32(capture_time_prop));
	this.Untag(raid_tag);
}
...
void onTick(CBlob@ this)
{
	...
	{
		int ticks = this.get_f32(counter_prop);
		const f32 captureTime = this.get_f32(capture_time_prop);
		...
		this.set_f32(counter_prop, ticks);
		this.Tag(raid_tag);
		...
		{
			this.set_f32(friendly_prop, friendlyCount);
			this.set_f32(enemy_prop, attackersCount);
...
void onRender(CSprite@ this)
{
	...
	const f32 friendlyCount = blob.get_f32(friendly_prop);
	const f32 enemyCount = blob.get_f32(enemy_prop);
	const f32 ticks = blob.get_f32(counter_prop);
	const f32 captureTime = blob.get_f32(capture_time_prop);
