	        ...
	        !this.isKeyPressed(key_right))
	{
		...
		for (uint i = 0; i < blobsInRadius.length; i++)
		{
					...
					!this.getMap().rayCastSolid(this.getPosition(), blob.getPosition()))
			{
				...
				if (stats_loaded && stats.id == Perks::bull && !blob.hasTag("weapon"))
				{
    			    continue;
    			}
...
void GatherAttachments(CBlob@ blob, AttachmentPoint@[]@ points)
{
	...
	for (int i = 0; i < count; i++)
	{
		...
		if ((ap.getOccupied() is null || ap.getOccupied().isBot()) && ap.socket && ap.getKeysToTake() > 0 && (!driver || (driver && !blob.isAttached()))) // gather empty controllers attachments/seats
		{
			points.push_back(@ap);
		}
	}
