...
f32 getAngle(CBlob@ this, const u8 charge, VehicleInfo@ v)
{
	...
	{
		Vec2f aim_vec = this.getPosition()-Vec2f(0,24) - gunner.getAimPos();
		        ...
		        (facing_left && aim_vec.x > 0))
		{
			...
			aim_vec.RotateBy(Maths::Floor((facing_left ? 1 : -1) * this.getAngleDegrees()));
...
void onTick(CBlob@ this)
{
	...
	if (this.hasAttached() || this.getTickSinceCreated() < 30)
	{
		...
		if (!broken && (fl ? currentAngle >= 270+10 : currentAngle <= 90-10)) Vehicle_StandardControls(this, v);
	...
	{
		arm.ResetTransform();
		arm.RotateBy(this.get_f32("gunelevation"), fl ? Vec2f(7.0f, 12.0f) : Vec2f(-7.0f, 12.0f));
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _charge)
{
	...
	if (bullet !is null)
	{
		...
		Vec2f vel = Vec2f(0.0f, -30.0f-XORRandom(31)*0.1f).RotateBy(angle);
		bullet.setVelocity(vel);
		Vec2f pos = this.getPosition() + Vec2f((fl ? -1 : 1)*16.0f, -22.0f).RotateBy((fl?angle+90:angle-90));
