	...
	if (this.hasAttached() || this.getTickSinceCreated() < 30)
	{
		...
		if (gunner !is null && gunner.getOccupied() !is null && !broken)
		{
			...
			if (p !is null && p.get("PerkStats", @stats))
			{
				...
				high_angle = 30.0f - stats.top_angle;
				low_angle =  98.0f + stats.down_angle;
			}
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _charge)
{
	...
	if (bullet !is null)
	{
		...
		Vec2f pos = this.getPosition() + (this.isFacingLeft()?Vec2f(-13.0f,0):Vec2f(13.0f,0)) + Vec2f((this.isFacingLeft() ? -1 : 1)*38.0f, -12.0f).RotateBy((this.isFacingLeft()?angle+90:angle-90));
		...
		if (hull !is null)
		{
			hull.AddForce(Vec2f(hull.isFacingLeft() ? (recoil*5.0f) : (recoil*-5.0f), 0.0f).RotateBy((this.isFacingLeft()?this.get_f32("gunelevation")+90:this.get_f32("gunelevation")-90)));
		}
