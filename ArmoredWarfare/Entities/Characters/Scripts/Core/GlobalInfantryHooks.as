...
const f32 min_damage_from_rpg_base = 0.15f;
const f32 max_damage_from_rpg_base = 0.75f;
const f32 rpg_damage_falloff_per_tile_base = 0.015f;
...
f32 onHit(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitterBlob, u8 customData)
{
		...
		|| hitterBlob.hasTag("grenade") || hitterBlob.getName() == "c4")
	{
		...
		if (hitterBlob.hasTag("rpg") && !hitterBlob.hasTag("artillery"))
		{
			...
			damage = damage * Maths::Min(max_damage_from_rpg, Maths::Max(min_damage_from_rpg, max_damage_from_rpg - (rpg_damage_falloff_per_tile * (dist_tiles))));
		}
	...
	if (this.isOnGround() || this.isInWater() || this.isAttached() || this.isOnLadder() || this.hasTag("dead"))
	{
		if (this.hasTag("parachute"))
		{
			for (uint i = 0; i < 50; i ++)
			{
				...
				ParticlePixel(this.getPosition() - Vec2f(0, 30) + getRandomVelocity(90.0f, 10 + (XORRandom(20) / 10.0f), 25.0f), vel, getTeamColor(this.getTeamNum()), false, 119);
			}
		}
