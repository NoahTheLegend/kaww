			...
			if (blob.isAttachedToPoint("DRIVER"))
			{
				...
				{	
						...
						else
						{
							...
							if (target !is null)
							{
								...
								if (target.isAttached())
								{
									...
									if (listentogunner)
									{
										if (emotes) set_emote(blob, "attn", 10);
									}
									else{
									...
									if (emotes) set_emote(blob, "check", 10);
					...
					else
					{
						...
						if (emotes) set_emote(blob, "down", 10);
					}
				}
				else{
					repairsneeded = true;
				...
				if (repairsneeded)
				{
					if (tertiarytarget !is null && tertiarytarget.getName() == "repairstation")
					{
						if ((tertiarytarget.getPosition() - vehicle.getPosition()).getLength() > 10.0f)
						{
							...
							if (realemotes)
							{
								...
								if (getGameTime() % 30 == 0 && XORRandom(120) == 0)
								{
									if (XORRandom(2) == 0)
									{
										set_emote(blob, XORRandom(2) == 0 ? "builder" : "frown");
									}
									else{
						...
						if (repairstations.length > 0)
						{
							...
							{
								set_emote(blob, XORRandom(2) == 0 ? "builder" : "heal");
			...
			if (realemotes)
			{
				...
				if (target is null)
				{
					...
					if (XORRandom(300) == 0 && getGameTime() % 30 == 0)
					{
						if (XORRandom(2) == 0)
						{
							set_emote(blob, "troll");
						}
						else{
							set_emote(blob, "finger");
						}
					}
				}
			}
		...
		if (target !is null) // logic for target
		{
			...
			if (strategy == Strategy::seekheal)
			{
				...
				if (realemotes)
				{
					...
					{
						set_emote(blob, XORRandom(2) == 0 ? "heal" : "cry");
					}
				}
			}
			...
			else
			{
				if (blob.get_u16("secondarytarget") != 0) // we have a secondary objective we should be doing
				{
					...
					if (secondarytarget !is null)
					{
						...
						{
							if (emotes) set_emote(blob, "cog", 2);
							...
							if ((secondarytarget.getPosition() - blob.getPosition()).getLength() < 40 && secondarytarget.getAttachments() !is null)
							{
								...
								if (blob.get_u8("moveover") != 0)
								{
									...
									{
										//if (emotes) set_emote(blob, "right", 10);
										blob.setKeyPressed(key_right, true);
									}
									else // 0 == left
									{		
										//if (emotes) set_emote(blob, "left", 10);
										blob.setKeyPressed(key_left, true);
									}
						...
						{	
							if (emotes) set_emote(blob, "redflag", 10);
							...
							else
							{
								if (realemotes)
								{
									if (XORRandom(12) == 0)
									{
										if (XORRandom(2) == 0)
										{
											set_emote(blob, "smile");
										}
										else{
											set_emote(blob, "thumbsup");
										}
									}
								}
