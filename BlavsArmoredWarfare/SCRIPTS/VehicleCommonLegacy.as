...
{
	if (getNet().isServer() && (this.getInventory().getItemsCount() > 0 || v.infinite_ammo) && getMagBlob(this) is null && v.loaded_ammo == 0)
	{
	...
	if (canFireIgnoreFiring(this, v) && caller !is null)
	{
		...
		else
		{
			...
			if (loadedAmmo != 0) // shoot if ammo loaded
			{
				...
				if (isClient())
				{
					if (v.bullet_name == "ballista_bolt" || v.bullet_name == "rocket")
					{
						//SetScreenFlash(25, 255, 250, 240, 0.7f);

						ShakeScreen(75, 20, caller.getPosition());
					}
					if (v.bullet_name == "bullet")
					{
						ShakeScreen(32, 10, caller.getPosition());
					}
				}
			}
		}
...
void Vehicle_StandardControls(CBlob@ this, VehicleInfo@ v)
{
	...
	if (this.getAttachmentPoints(@aps))
	{
		for (uint i = 0; i < aps.length; i++)
		{
			...
			{
				// GET OUT
				if (blob.isMyPlayer() && ap.isKeyJustPressed(key_up) && ap.name != "FLYER")
				{
					CBitStream params;
				...
				if (ap.name == "DRIVER")
				{
					...
					const bool left = ap.isKeyPressed(key_action1) ? false : ap.isKeyPressed(key_left);
					const bool right = ap.isKeyPressed(key_action1) ? false : ap.isKeyPressed(key_right);
					...
					if (angle < 80 || angle > 290)
					{
						...
						if (this.getShape().vellen < 0.3f)
						{
							moveForce *= 2.0f;
						}

						// more force for truck accel
						if (this.getShape().vellen < 6.0f && this.getName() == "techtruck")
						{
							moveForce *= 1.4f;
						}
						...
						if (left)
						{
							...
							if (vel.x < -turnSpeed && !ap.isKeyPressed(key_action2))
							{
								this.SetFacingLeft(true);
							}

							if (!this.isFacingLeft() && ap.isKeyPressed(key_action2))
								force.x *= 0.7;
						...
						{
							this.getShape().setFriction(0.42f);
							...
							{
								if (Maths::Abs(this.getVelocity().x) < 0.35f)
									this.setVelocity(Vec2f(this.getVelocity().x * (this.isOnGround() || this.wasOnGround() ? 0.5f : 1.0f), this.getVelocity().y * (this.isOnGround() || this.wasOnGround() ? 0.5f : 1.0f)));	
								else
									this.setVelocity(Vec2f(this.getVelocity().x * (this.isOnGround() ? 0.5f : 1.0f), this.getVelocity().y * (this.isOnGround() ? 0.5f : 1.0f)));	
							}
							else
								this.setVelocity(Vec2f(this.getVelocity().x * (this.isOnGround() ? 0.92f : 1.0f), this.getVelocity().y * (this.isOnGround() ? 0.92f : 1.0f)));	
						}
						else
						{
							this.getShape().setFriction(0.02f);
						}
					...
					if (onground && (down || moveUp))
					{
						...
						if (angle > 330 || angle < 30)
						{
							...
							f32 torque = 40.0f * wallMultiplier; //150.0f * wallMultiplier
							if (down)
