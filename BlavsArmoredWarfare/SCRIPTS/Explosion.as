...
void WarfareExplode(CBlob@ this, f32 radius, f32 damage)
{
	...
	for (int i = 0; i < radius * 0.16; i++)
	{
		Vec2f partpos = thisPos + Vec2f(XORRandom(r * 2) - r, XORRandom(r * 2) - r);
		Vec2f endpos = partpos;

		if (map !is null)
		{
			if (!map.rayCastSolid(thisPos, partpos, endpos))
				makeSmallExplosionParticle(endpos);
		}
	}
	...
	if (isServer())
	{
        Vec2f m_pos = (thisPos / map.tilesize);
        m_pos.x = Maths::Floor(m_pos.x);
        m_pos.y = Maths::Floor(m_pos.y);
        m_pos = (m_pos * map.tilesize) + Vec2f(map.tilesize / 2, map.tilesize / 2);
		...
		//hit blobs
		CBlob@[] blobs;
		map.getBlobsInRadius(thisPos, radius, @blobs);
...
void onHitBlob(CBlob@ this, Vec2f worldPoint, Vec2f velocity, f32 damage, CBlob@ hitBlob, u8 customData)
{
	...
	{
		hitBlob.AddForce(Vec2f(Maths::Min(velocity.x * 4.5f, Maths::Min(1, Maths::Max(-1, velocity.x)) * 260.0f), Maths::Min(velocity.y * 1.5f, Maths::Min(1, Maths::Max(-1, velocity.y)) * 150.0f)));
...
bool WarfareHitBlob(CBlob@ this, CBlob@ hit_blob, f32 radius, f32 damage, const u8 hitter, const bool should_teamkill = false)
{
	Vec2f pos = this.getPosition();
	CMap@ map = this.getMap();
	Vec2f hit_blob_pos = hit_blob.getPosition();
