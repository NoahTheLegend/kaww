...
{
	this.set_u32("mag_bullets_max", 5); // mag size

	this.set_u32("mag_bullets", 5);
...
void ClientFire(CBlob@ this, const s8 charge_time)
{
	...
	Vec2f(this.isFacingLeft() ? 1.0f : -1.0f, -4.0f), // velocity
	0,                                  // column
	0,                                  // row
		...
		if (p !is null)
		{
			...
			if (local !is null)
			{
				...
				if (ply !is null && ply.isMyPlayer())
				{
					...
					ShakeScreen(40, 10, this.getPosition());

					this.set_u8("inaccuracy", Maths::Min(250, this.get_u8("inaccuracy") + inaccuracypershot * (this.hasTag("sprinting")?2.0f:1.0f)));
				}
			}
		}
	}
...
void onCommand(CBlob@ this, u8 cmd, CBitStream @params)
{
	if (cmd == this.getCommandID("shoot bullet"))
	{
		...
		this.getSprite().PlaySound(shootsfx, 1.25f, 0.90f + XORRandom(25) * 0.01f);
	}
	else if (cmd == this.getCommandID("sync_reload_to_server"))
	{
		if (this.get_s8("charge_time") == 0 || this.get_s8("charge_time") > 60) this.getSprite().PlaySound(reloadsfx, 0.8, 0.8);
