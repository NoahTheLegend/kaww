...
void ManageBow(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	else
	{
		...
		{
			moveVars.walkFactor *= this.getPlayer().hasTag("Conditioning") ? 1.2f : 1.1f;
	...
	if (hasarrow && canSend(this))  // client-logic
	{
		...
		if (p !is null)
		{
			...
			if (local !is null)
			{
				...
				{
					Recoil(this, local, 2.0f, 1);

					ShakeScreen(20, 6, this.getPosition());
...
{ if (this is null) return;
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		this.getSprite().PlaySound("PistolFire.ogg");
