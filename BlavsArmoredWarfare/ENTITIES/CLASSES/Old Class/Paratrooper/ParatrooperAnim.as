...
void LoadSprites(CSprite@ this)
{
	...
	case PLAYER_ARMOUR_STANDARD:
		ensureCorrectRunnerTexture(this, "paratrooper", "Paratrooper");
		break;
	...
	CSpriteLayer@ parachute = this.addSpriteLayer("parachute",   64, 64);
...
void onTick(CSprite@ this)
{
	...
	if (parachute !is null)
	{
		...
		parachute.SetOffset(Vec2f(0.0f, - 30.0f + Maths::Sin(getGameTime() / 5.0f) * 2.0f));

		int sign = parachute.isFacingLeft() ? 1 : -1;
		f32 parachute_angle = (Maths::Round((Maths::Sin(getGameTime() / 7.0f) * 4.0f) * 1) % 360 ) / 1.0f;
		parachute.ResetTransform();
		parachute.RotateBy(parachute_angle + 0.5f, Vec2f(0.5*sign, 3.5));

		if (blob.hasTag("parachute"))
...
void DrawBow(CSprite@ this, CBlob@ blob, ArcherInfo@ archer, f32 armangle, const u8 arrowType, Vec2f armOffset)
{
	...
	if (archer.charge_time == 6)
	{
		frontarm.animation.frame = 4;
		setArmValues(frontarm, true, armangle, 0.1f, "fired", Vec2f(-4.0f * sign, 0.0f), armOffset*2);
	}
	else
	{
		frontarm.animation.frame = 4;
