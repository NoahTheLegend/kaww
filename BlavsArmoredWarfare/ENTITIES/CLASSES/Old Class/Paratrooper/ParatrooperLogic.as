...
void ManageBow(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	if (this.hasTag("parachute"))
	{
		this.setVelocity(Vec2f(Maths::Abs(this.getVelocity().x) < 3.5f ? this.getVelocity().x * 1.1f : this.getVelocity().x * 1.0f, this.getVelocity().y * (0.68f + (Maths::Sin(getGameTime() / 6.5f) * 2.0f)/8))); //0.68   //11
	}

	// Do the stab
	if (charge_time == 11 && archer.isStabbing)
	{
		f32 attackarc = 95.0f;
	...
	else if (this.isKeyPressed(key_action1))
	{
		...
		if (charge_time == 0 && isStabbing == false && (this.hasTag("parachute") || this.isOnGround() || this.wasOnGround() || this.isOnLadder() || this.wasOnLadder()))
		{
	...
	else
	{
		...
		if (this.getPlayer() !is null)
		{
			...
			moveVars.jumpFactor *= this.getPlayer().hasTag("Conditioning") ? 0.9f : 0.8f;
		}
	}
...
void ClientFire(CBlob@ this, const s8 charge_time, const bool hasarrow, const u8 arrow_type, const bool legolas)
{
	...
	if (hasarrow && canSend(this))  // client-logic
	{
		...
		{
			ShootArrow(this, this.getPosition(), this.getAimPos() + Vec2f(-7.0f + XORRandom(14), -7.0f + XORRandom(14)), ArcherParams::shoot_max_vel*2.1f, ArrowType::normal, legolas);
		}
		else
		{
			ShootArrow(this, this.getPosition(), this.getAimPos() + Vec2f(-14.0f + XORRandom(28), -16.0f + XORRandom(28)), ArcherParams::shoot_max_vel*2.1f, ArrowType::normal, legolas);
		}
		...
		if (p !is null)
		{
			...
			if (local !is null)
			{
				...
				if (ply !is null && ply.isMyPlayer())
				{
					...
					ShakeScreen(24, 8, this.getPosition());
...
{ if (this is null) return;
	if (cmd == this.getCommandID("shoot arrow"))
	{
		...
		this.getSprite().PlaySound("RifleFirePara.ogg");
