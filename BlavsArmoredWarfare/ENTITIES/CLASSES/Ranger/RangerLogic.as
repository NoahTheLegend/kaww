	...
	if (map.getHitInfosFromArc(pos, aimangle, arcdegrees, radius + attack_distance, this, @hitInfos))
	{
		...
		for (uint i = 0; i < hitInfos.length; i++)
		{
			...
			if (b !is null) // blob
			{
				...
				if (b.getName() == "wooden_platform" || b.hasTag("door")) damage *= 2;
...
void ManageGun(CBlob@ this, ArcherInfo@ archer, RunnerMoveVars@ moveVars)
{
	...
	{
		// reload
		if (charge_time == 0 && controls !is null && !archer.isReloading && controls.isKeyJustPressed(KEY_KEY_R) && !is_action1 && this.get_u32("mag_bullets") < this.get_u32("mag_bullets_max"))
		{
			//print("RELOAD!!");
	...
	if (canSend(this))
	{
		...
		if (p !is null)
		{
			...
			if (local !is null)
			{
				...
				{
					f32 mod = 0.5f; // make some smart stuff here?
					if (this.isKeyPressed(key_action2)) mod *= 0.35;
					...
					if (!this.isBot())
					{
						...
						//this.set_s8("recoil_direction", ((Maths::Sin(getGameTime()*0.075)/0.03f) + (30 - XORRandom(61))) / sidewaysrecoildamp);
					}
