...
void onInit(CBlob@ this)
{
	...
	// init launcher sprites
	CSprite@ sprite = this.getSprite();
	CSpriteLayer@ launcher = sprite.addSpriteLayer("launcher", sprite.getConsts().filename, 24, 80);
	...
	if (launcher !is null)
	{
		...
		Animation@ anim = launcher.addAnimation("default", 0, false);
		anim.AddFrame(20);

		CSpriteLayer@ launcher = this.getSprite().getSpriteLayer("launcher");
		if (launcher !is null)
		{
			launcher.SetRelativeZ(-2.5f);
			launcher.SetOffset(Vec2f(-32.0f, 2.0f));
		}
	}
...
void onTick(CBlob@ this)
{
	...
	CSprite@ sprite = this.getSprite();
	CSpriteLayer@ launcher = sprite.getSpriteLayer("launcher");
	if (launcher !is null)
	{
		launcher.ResetTransform();
		launcher.RotateBy(this.get_f32("gunelevation"), Vec2f(-0.5f, 15.5f));
		launcher.SetOffset(Vec2f(-12.0f, -26.0f + (this.isFacingLeft() ? -1.0f : -0.0f)));
		launcher.SetOffset(launcher.getOffset() - Vec2f(-barrel_compression + Maths::Min(v.getCurrentAmmo().fire_delay - v.cooldown_time, barrel_compression), 0).RotateBy(this.isFacingLeft() ? 90+this.get_f32("gunelevation") : 90-this.get_f32("gunelevation")));
		launcher.SetRelativeZ(-50.0f);
	}
}
...
void Vehicle_onFire(CBlob@ this, VehicleInfo@ v, CBlob@ bullet, const u8 _charge)
{
	...
	if (bullet !is null)
	{
		...
		Vec2f pos = this.getPosition() + Vec2f((this.isFacingLeft() ? -1 : 1)*38.0f, -11.0f).RotateBy((this.isFacingLeft()?angle+90:angle-90));
		...
		if (isClient())
		{
			...
			for (int i = 0; i < 4; i++)
			{
				...
				ParticleAnimated("Explosion", pos, this.getShape().getVelocity() + getRandomVelocity(0.0f, XORRandom(45) * 0.005f, 360) + vel/(40+XORRandom(24)), float(XORRandom(360)), 0.3f + XORRandom(20) * 0.01f, 3, -0.0031f, true);
			}
		}
